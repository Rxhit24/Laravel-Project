<!-- <template>
    <div class="card border-0 shadow">
        <div class="card-body border-2 border-primary">
            ...
            <div v-if="errorMsg" class="bg-danger w-100 ">
                {{ errorMsg }}
            ...
            <form @submit.prevent="login">
                <input type="text" v-model="user.email" autocomplete="email" class="form-control my-4 py-2" placeholder="Email" />
                <input type="text" v-model="user.password" autocomplete="current-password" class="form-control my-4 py-2" placeholder="Password" />
                <div class="text-center mt-3">
                    <button class="btn btn-primary bg-primary my-3" >Login</button>
                    <router-link :to="{ name: 'Register' }" class="nav-link">Don't have an account</router-link>
                </div>
            </form>
        </div>
    </div>
...
<template>
    ...
    <form class="mt-8 space-y-6" @submit.prevent="login">
        <Alert v-if="errorMsg">
            <p>User Email Or Password Does Not Match!</p>
            <span @click="errorMsg = ''"
